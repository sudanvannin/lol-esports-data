"""Upcoming match helpers for the web app.

Prefer the shared implementation from ``src.upcoming_matches`` when available.
Render's web image may not include the full ``src`` tree, so keep a local
...
try:
    from src.upcoming_matches import (
        load_upcoming_matches,
        normalize_upcoming_match_rows,
    )
    ...
    def _extract_league_name(overview_page: str, match_id: str) -> str:
        for candidate in (
            _clean_text(overview_page),
            _clean_text(match_id).split("_")[0],
        ):
            if not candidate:
                continue
            if "/" in candidate:
        ...
        *,
        ...
        for row in rows:
            ...
            if not _is_blank(row.get("Winner")):
                continue
            if not _is_blank(row.get("Team1Score")) or not _is_blank(
                row.get("Team2Score")
            ):
                continue
    ...
    def load_upcoming_matches(
        ...
        *,
        ...
        """Load Leaguepedia schedule payload and return normalized rows plus metadata."""
        if not path.exists():
        ...
        return (
            normalize_upcoming_match_rows(rows, reference_time=reference_time),
            metadata,
        )
