...
def upload_to_motherduck(token: str, db_name: str = "lolesports"):
    ...
    try:
        ...
        if not silver_dir.exists():
            logger.error(
                f"Silver directory not found at {silver_dir}. Run processing first."
            )
            return False
        ...
        if players_dir.exists() and any(players_dir.iterdir()):
            ...
            con.execute(
                f"CREATE OR REPLACE TABLE players AS SELECT * FROM '{players_path}'"
            )
        else:
        ...
        if games_dir.exists() and any(games_dir.iterdir()):
            ...
            con.execute(
                f"CREATE OR REPLACE TABLE games AS SELECT * FROM '{games_path}'"
            )
        else:
        ...
        if series_path.exists():
            ...
            con.execute(
                f"CREATE OR REPLACE TABLE series AS SELECT * FROM '{series_path}'"
            )
        ...
        if champs_path.exists():
            ...
            con.execute(
                f"CREATE OR REPLACE TABLE champions AS SELECT * FROM '{champs_path}'"
            )
        ...
        if gold_pointer.exists():
            ...
            gold_tables = {
                ...
                "gold_external_reconciliation": snapshot_dir
                / "external_reconciliation.parquet",
                ...
                "gold_match_features_prematch": snapshot_dir
                / "match_features_prematch.parquet",
            ...
            for table_name, table_path in gold_tables.items():
                if table_path.exists():
                    ...
                    logger.info(
                        f"Skipping '{table_name}' upload (missing file: {table_path.name})."
                    )
        else:
            logger.info(
                "Skipping Gold upload (data/gold/latest_snapshot.json not found)."
            )
        ...
        con.execute(
            "CREATE OR REPLACE TABLE web_upcoming_matches AS SELECT * FROM web_upcoming_matches_df"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--token",
        type=str,
        help="MotherDuck API Token (or use MOTHERDUCK_TOKEN env var)",
    )
    parser.add_argument(
        "--db", type=str, default="lolesports", help="MotherDuck Database Name"
    )
    ...
    if not token:
        logger.error(
            "MotherDuck token not provided. Use --token or MOTHERDUCK_TOKEN env var."
        )
        sys.exit(1)
